class AcompanhamentoProntuarioPresencaPdf < BasePdf
  ...
  def body
    ...
    @acompanhamento.atendimentos.each_with_index do |atendimento, index|
      str += dados_atendimento atendimento, index: index + 1
    end.join " "
    str += "</tbody>\n" \
      "</table>" \
      ""
  ...
  def dados_cadastrais
    ...
    markup "#{dados.map { |k,v| "<b>#{k&.to_s&.humanize}</b>: #{v&.to_s&.upcase}" }.to_sentence(words_connector: "<br>", last_word_connector: "<br>")}"
    move_down 6
    horizontal_rule
    move_down 12
  end
  ...
  def dados_atendimento atendimento, index: 0
    "<tr>" \
      "<td>#{index}</td>" \
      "<td>#{atendimento.data_inicio_verdadeira.strftime("%d/%m/%Y")}</td>" \
      "<td>#{atendimento.horario_inicio_verdadeiro.strftime("%H:%M")}</td>" \
      "<td>#{atendimento.tipo}</td>" \
      "<td>#{atendimento.status}</td>" \
      "</tr>"
  end

  def dados_atendimento_alt atendimento, instrumento_relatos: @instrumento_relatos, index: 0
      str = ""
