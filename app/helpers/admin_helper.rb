module AdminHelper
  ...
  def edit_attribute table_name, id, attribute, value
  ...
  def edit_assoc table_name, id, attribute, assoc, blank_option: "NENHUM"
    ...
    hx_form post: admin_update_path(table: table_name, id: id, edit: attribute, authenticity_token: form_authenticity_token.to_s), select: final_id, target: final_id, swap: "outerHTML" do
      ...
      label_inner = main_class.validators_on(main_class.foreign_key_associations[attribute]).map(&:options).map { |h| h[:message] }.join(" ")
      str += "<label for=\"#{param_name}\" style=\"#{html_style}\">#{label_inner}</label>" if valids.any?
